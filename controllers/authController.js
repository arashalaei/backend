...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 3) if eerything ok, send token to client
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    if (!roles.includes(req.user.role))
      return next(
        new AppError("You dont have permisson to perrfoem this action", 403)
      );
    next();
  };
};
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const resetToken = user.createPasswordResetTokken();
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) if token has has not expired,and there is user, set the new passwprd
